...
public class Card extends CardType {
	...
	public List<Card> getPossibleRotations() {
		...
		case I:
			...
			break;
		default:
			System.err.print(Messages.getInstance().getString("Card.invalidShape")); //$NON-NLS-1$
			break;
		}
		return cards;
	}
	...
	public boolean equals(Object obj) {
		...
		if (this.treasure != other.getTreasure()) {
			Debug.print(Messages.getInstance().getString("Card.treasureUnequal"), DebugLevel.DEBUG); //$NON-NLS-1$
			return false;
		}
		for (Integer ID : this.getPin().getPlayerID()) {
			if (!other.getPin().getPlayerID().contains(ID)) {
				Debug.print(Messages.getInstance().getString("Card.playerUnequal"), DebugLevel.DEBUG); //$NON-NLS-1$
				return false;
			}
		}
		if (other.getShape() != this.getShape()) {
			Debug.print(Messages.getInstance().getString("Card.shapeUnequal"), DebugLevel.DEBUG); //$NON-NLS-1$
			return false;
		}
		return true;
	}
